    ...
    private void UpdateFields()
    {
        for(int i=0; i<electricField.Length; i++)
        {
            ...
            for(int j=0; j<chargePositions.Count; j++)
            {
                ...
                electricField[i] += chargePositions[j].displayCharge / r2 * direction.normalized;
            }
        }
    ...
    void FixedUpdate()
    {
        for(int i=0; i<particlePositions.Count; i++)
        {
            for(int j=0; j<chargePositions.Count; j++)
            {
                ...
                float r2 = direction.sqrMagnitude;
                particleVelocities[i] += forceIntensities * chargePositions[j].displayCharge * particleElements[i].charge / r2 * direction.normalized * Time.fixedDeltaTime;
            }
            for(int j=0; j<magneticPositions.Count; j++)
            {
                ...
                float r = direction.magnitude;
                Vector3 force = Vector3.Cross(magneticPositions[j].displayCharge * particleElements[i].charge / r * magneticPositions[j].transform.up, new Vector3(particleVelocities[i].x, 0, particleVelocities[i].y));
